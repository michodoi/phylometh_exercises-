...
GetTreeFromOpenTree <- function() {
 ...
 #formica.id <- tnrs_match_names("__________")$ott_id
  Lamniformes.id <-tnrs_match_names("Lamniformes")$ott_id
  ...
  Lamniformes.tree <- tol_subtree(ott_id=Lamniformes.id)

	# Let's plot the tree:
	plot.phylo(Lamniformes.tree, type="fan", cex=0.2)
	print(paste("The Lamniformes tree has ", Ntip(Lamniformes.tree), " terminals and ",
	Nnode(Lamniformes.tree), " internal nodes out of ",Ntip(Lamniformes.tree)-2,
	" possible, which means it is ",
	round(100*(Nnode(Lamniformes.tree)-1)/(Ntip(Lamniformes.tree)-3), 2),
	"% resolved", sep=""))

	# Open Tree can also return the original studies with the source trees.
# 	Lamniformes.studies <- studies_find_studies(property="ot:focalCladeOTTTaxonName",
# 	value="Lamniformes")
# 	Lamniformes.studies.ids <- unlist(Lamniformes.studies$study_ids)
# 
# 	# Let's get info on the first study
# 	Lamniformes.study1.metadata <- get_study_meta(Lamniformes.studies[[1]][[1]])
# 	print(get_publication(Lamniformes.study1.metadata))
# 
...
#   Lamniformes.study1.tree1 <- get_______FUNCTION_FROM_rotl____(Lamniformes.studies.ids[[1]])
# 
# 	# And plot it
# 	plot.phylo(Lamniformes.study1.tree1, type="fan", cex=0.2)

Lamniformes.study.metadata<-get_study_meta("ot_1959")
Lamniformes.tree.ids<-get_tree_ids(Lamniformes.study.metadata)
Lamniformes.tree.ids[[1]]
                    
Lamniformes.study1.tree1 <- get_study_tree("ot_1959", Lamniformes.tree.ids[[1]])

	#Return both trees
	return.list <- list(Lamniformes.tree, Lamniformes.study1.tree1)
